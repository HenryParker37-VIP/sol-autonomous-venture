...
def dashboard_snapshot() -> dict:
    with connect() as c:
        ...
        event_counts = {row["event_type"]: row["count"] for row in c.execute("SELECT event_type,COUNT(*) AS count FROM events WHERE actor NOT LIKE 'sandbox%' GROUP BY event_type")}
        referral_visits = sum((row["visits"] or 0) for row in c.execute("SELECT visits FROM distribution_metrics WHERE visits IS NOT NULL"))
        cta_clicks = sum((row["clicks"] or 0) for row in c.execute("SELECT clicks FROM distribution_metrics WHERE clicks IS NOT NULL"))
        post_impressions = sum((row["impressions"] or 0) for row in c.execute("SELECT impressions FROM distribution_metrics WHERE impressions IS NOT NULL"))
        ...
        acquisition_metrics = {"external_posts_published": event_counts.get("EXTERNAL_POST_PUBLISHED", 0), "post_reactions_received": event_counts.get("POST_REACTION_RECEIVED", 0), "post_impressions": post_impressions, "profile_views": 0, "outreach_sent": event_counts.get("OUTREACH_SENT", 0), "replies_received": event_counts.get("REPLY_RECEIVED", 0), "referral_visits": referral_visits, "cta_clicks": cta_clicks, "form_starts": event_counts.get("FORM_STARTED", 0), "orders_created": funnel["orders"], "verified_payments": funnel["paid_orders"], "sales": funnel["paid_orders"]}
        return {"state": state, "milestones": milestones, "agents": agents, "worker_status": worker_status, "tasks": tasks, "events": live_events, "sandbox_events": sandbox_events, "orders": orders, "sandbox_orders": sandbox_orders, "opportunities": opportunities, "product": dict(product) if product else None, "publication": dict(publication) if publication else None, "distribution_metrics": distribution_metrics, "acquisition_metrics": acquisition_metrics, "funnel": funnel, "referral_sources": referral_sources, "feedback": feedback, "acquisition": acquisition_config(), "intake": intake, "financial": {"cost_usd": costs, "revenue_usd": revenue, "net_usd": revenue-costs, "budget_remaining_usd": max(0, 3-costs), "buyers": buyers}}
